...
- Naive implementation: This version performs the transpose by reading each element from the input matrix in row-major order and writing it directly to the transposed position in the output matrix. While the reads from global memory are coalesced, the writes to global memory are not — consecutive threads in a wavefront write to addresses separated by N × 4 bytes, resulting in one cache line transaction per thread instead of one shared transaction per wavefront.
- Shared memory implementation: This version uses a tile of Local Data Share (LDS) memory of size 16 × 17 elements as a staging buffer. In the first phase, threads load a 16 × 16 tile from the input matrix into LDS in a coalesced, row-major fashion. After a synchronization barrier, threads re-map their roles to write the transposed tile to the output matrix in a coalesced, row-major fashion as well. The extra column of padding in the LDS tile (17 instead of 16) prevents bank conflicts during the LDS read phase.
